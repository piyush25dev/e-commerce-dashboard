// components/admin/about/TeamSection.jsx
...
function TeamRow({ member, index, onEdit, onDelete }) {
  return (
    <TableRow>
      ...
      <TableCell sx={{ borderColor: "#ffffff0a" }}>
        <Stack direction="row" alignItems="center" spacing={1.5}>
          <Typography sx={{ fontSize: "1.5rem" }}>{member.avatar}</Typography>
          <Box>
...
export default function TeamSection({ team, loading, onRefresh }) {
  ...
  const handleSave = async () => {
    if (!name.trim() || !role.trim() || !avatar.trim()) return;
    ...
    try {
      ...
      if (editingId) {
        await updateDoc(doc(db, "aboutTeam", editingId), data);
      } else {
        await addDoc(collection(db, "aboutTeam"), data);
      }
      setDialogOpen(false);
      resetForm();
      onRefresh();
  ...
  const handleDelete = async (id) => {
    try {
      await deleteDoc(doc(db, "aboutTeam", id));
  ...
  return (
    <Box>
      ...
      <Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ sm: "center" }} mb={3} spacing={2}>
        <Box>
          <Typography variant="h5" color="text.primary">Team Members</Typography>
          <Typography variant="body2" color="text.secondary">
            {team.length} member{team.length !== 1 ? "s" : ""} total
          </Typography>
        </Box>
                ...
                <TableRow>
                  <TableCell colSpan={4} align="center" sx={{ py: 8, borderColor: "#ffffff0a" }}>
                    ...
                    <Typography color="text.secondary" variant="body2">
                      {search ? "No members match your search." : "No members yet. Add one!"}
                    </Typography>
                  </TableCell>
                </TableRow>
              ) : (
      ...
      >
        ...
        <DialogContent sx={{ pt: 2.5 }}>
          <Stack spacing={2.5}>
            ...
            {avatar && (
              ...
              >
                <Typography sx={{ fontSize: "3rem" }}>{avatar}</Typography>
                <Box>
                  <Typography variant="body2" fontWeight={700} color="text.primary">
                    {name || "Team Member"}
                  </Typography>
        ...
        <DialogActions sx={{ px: 3, py: 2 }}>
          ...
          <Button
            variant="contained"
            onClick={handleSave}
            disabled={!name.trim() || !role.trim() || !avatar.trim() || saving}
      ...
      >
        <DialogTitle>
          <Typography fontWeight={700}>Delete Member?</Typography>
        </DialogTitle>
        <DialogContent>
          <Typography color="text.secondary" variant="body2">
            This action cannot be undone. The member will be permanently removed from Firestore.
          </Typography>
        </DialogContent>
